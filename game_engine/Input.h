...
public:
	...
	static inline bool GetKeyWrapper(const std::string& key) {
		auto scancode = __keycode_to_scancode.find(key);
		if (scancode != __keycode_to_scancode.end()) {
			return GetKey(scancode->second);
		}
		return false;
	}

	static inline bool GetKeyDownWrapper(const std::string& key) {
		auto scancode = __keycode_to_scancode.find(key);
		if (scancode != __keycode_to_scancode.end()) {
			return GetKeyDown(scancode->second);
		}
		return false;
	}

	static inline bool GetKeyUpWrapper(const std::string& key) {
		auto scancode = __keycode_to_scancode.find(key);
		if (scancode != __keycode_to_scancode.end()) {
			return GetKeyUp(scancode->second);
		}
		return false;
	}

	static inline bool GetMouseButtonWrapper(int button) {
		if (button >= 1 && button <= 3) {
			return GetMouseButton(button); // Assuming 0-indexed
		}
		return false;
	}

	static inline bool GetMouseButtonDownWrapper(int button) {
		if (button >= 1 && button <= 3) {
			return GetMouseButtonDown(button); // Assuming 0-indexed
		}
		return false;
	}

	static inline bool GetMouseButtonUpWrapper(int button) {
		if (button >= 1 && button <= 3) {
			return GetMouseButtonUp(button); // Assuming 0-indexed
		}
		return false;
	}
